...
def configurarRdf():
     db = pymysql.connect(host='localhost', user='root', password='', db='data')
     cur = db.cursor()
     #sql ="""SELECT DISTINCT concat('<', sujeto,'> <', predicado,'> <', objeto,'>.') FROM tripletas WHERE objeto LIKE 'http%://%' UNION SELECT DISTINCT concat('<', sujeto, '> <', predicado, '> "', REPLACE(REPLACE(REPLACE(REPLACE(objeto, '"', '\''),' ', ' '), '\\' , ''), ' ', ''), '".') FROM tripletas WHERE objeto NOT LIKE 'http://' OR objeto NOT LIKE 'https://' """
     meetId = sys.argv[1]

     #cur.execute (sql, (meetId,))
     cur.execute('SELECT DISTINCT concat('<', sujeto,'> <', predicado,'> <', objeto,'>.') FROM tripletas WHERE objeto LIKE 'http%://%' UNION SELECT DISTINCT concat('<', sujeto, '> <', predicado, '> "', REPLACE(REPLACE(REPLACE(REPLACE(objeto, '"', '\''),' ', ' '), '\\' , ''), ' ', ''), '".') FROM tripletas WHERE objeto NOT LIKE 'http://' OR objeto NOT LIKE 'https://' ')
...
'''
    	select=input ("Seleccione si el objeto es un literal (l) o una URI (u)") 
    	#input (select)
     if (select== "l"):
    	sql= """ SELECT concat ('<',sujeto, '> <', predicado, '> "', objeto, '".') FROM tripletas; """
    	cur.execute (sql)
    	for fila in cur:
    		print (fila)
    	db.close()
